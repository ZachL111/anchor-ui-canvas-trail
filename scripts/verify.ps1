...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-anchor-ui-canvas-detail.ps1

lua tests/test_domain_review.lua

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-anchor-ui-canvas-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-anchor-ui-canvas-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-anchor_ui_canvas_trail-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
